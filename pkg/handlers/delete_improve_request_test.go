...
func TestDeleteImproveRequestHandler(t *testing.T) {
	data := []struct {
		name string
	...
	for _, d := range data {
		t.Run(d.name, func(t *testing.T) {
			service := servicesmocks.NewDeleteImproveRequestService(t)
			...
			handler := handlers.NewDeleteImproveRequestHandler(service)
			handler.Handle(c)
